...
(function ($) {
  ...
  $(document).on("click", 'a[href^="#"]', function (event) {
    ...
    if (target.length) {
      ...
      if (smoother && window.innerWidth >= 1024) {
        ...
        var topPos = target.offset().top - 80;
        window.scrollTo({ top: topPos, behavior: "smooth" });
      }
    }
  });

  // Kill any residual smoother to restore 100% native 60fps/120fps hardware-accelerated scrolling
  ...
  hoverItem.forEach((item) => {
    item.addEventListener(
      "mousemove",
      (e) => {
        window.requestAnimationFrame(() => moveImage(e, item, 1));
      },
      { passive: true }
    );
  });
  ...
  ///////////////////////
  // 11. Portfolio Three Shape Pinning is handled natively by CSS position: sticky for 120fps lag-free rendering
})(jQuery);
